...
tags_url="https://registry.hub.docker.com/v2/repositories/${image_name}/tags?page=1&page_size=150"
...
last_tag=$(curl -sSL "${tags_url}"|jq --arg match_repr "${!jq_var}" -r '.results[]|select(.name|match($match_repr))|.name'|sort -V|tail -1)
